...
function M.enter()
  ...
  pcall(function()
    session:scroll_to_source_line(source_cursor)
  end)
  ...
  vim.keymap.set("n", "<CR>", function()
    M.jump_from_outline(source_buf)
  end, { buffer = outline_buf, desc = "mdread: jump to heading", silent = true })
  vim.keymap.set("n", "<2-LeftMouse>", function()
    M.jump_from_outline(source_buf)
  end, { buffer = outline_buf, desc = "mdread: jump to heading", silent = true })
  vim.keymap.set("n", "q", function()
    M.exit(source_buf)
  end, { buffer = outline_buf, desc = "mdread: exit", silent = true })
  vim.keymap.set("n", "q", function()
    M.exit(source_buf)
  end, { buffer = session.buf, desc = "mdread: exit", silent = true })

  vim.api.nvim_create_autocmd({ "VimResized", "WinResized" }, {
    group = augroup,
    callback = function()
      M.on_resize(source_buf)
    end,
  })
...
function M.setup(opts)
  opts = opts or {}
  if opts.outline_width then
    config.outline_width = opts.outline_width
  end
end
